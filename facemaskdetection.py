...
while True:
    ...
    for(x, y, w, h) in face:
        ...
        if model.predict(img)[0][0]>0.5:
            frame = cv2.putText(frame, 'Unwear mask!!!', (x, y), cv2.FONT_HERSHEY_SIMPLEX,1, (0, 0, 255), 2, cv2.LINE_AA)
        else:
            frame = cv2.putText(frame, 'Wear mask', (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
